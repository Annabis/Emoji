...
const sectionElement = document.querySelector("#emojis");
...
const renderEmojis = (result) => {
  sectionElement.innerHTML = "";
  result.forEach((item) => {
    const emojiElement = templateElement.content.cloneNode(true);
    const symbolElement = emojiElement.querySelector(".emoji__symbol");
    const titleElement = emojiElement.querySelector(".emoji__title");
    const keywordsElement = emojiElement.querySelector(".emoji__keywords");
    symbolElement.textContent = item.symbol;
    titleElement.textContent = item.title;
    keywordsElement.textContent = item.keywords;

    sectionElement.append(emojiElement);
  });
};

inputElement.addEventListener("input", (event) => {
  let value = event.target.value;
  renderEmojis(handleSerch(value));
});
